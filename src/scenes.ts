...
function cottage(p: Plan, cx: number, gy: number, s: number, occupant = false) {
  const P = (pts: Pt[]) => place(pts, cx, gy, s);
  const R = (x0: number, y0: number, x1: number, y1: number) => P(rect(x0, y0, x1, y1));
  const L = (segs: [number, number, number, number][]) => segs.map(([a, b, c, d]) => P([[a, b], [c, d]]));
  ...
  p.clear(win, .95, 4 * s);
  if (occupant) {
    p.fill(spline(P([[80, -70], [83, -80], [90, -85], [100, -87], [110, -85], [117, -80], [120, -70]]), true, 3), .92, 3 * s);
    p.fill(P(ellipse(100, -99, 8.5, 10, 24)), .95, 2.5 * s);
    p.fill(P(ellipse(106, -109, 5, 4.5, 16)), .95, 2 * s);
  }
